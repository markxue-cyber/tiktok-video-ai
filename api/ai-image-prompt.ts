        ...
        messages: [
          {
            ...
            content: [
              ...
              '- 目标参考构图：1) 白底/浅灰棚拍的结构清晰主图；2) 旋转手柄、可拆电池、喷嘴切换等功能信息图式构图（可用图标与箭头样式，默认不强制可读文字，优先预留后期贴字区）；3) 户外落叶清理使用场景（可含真人全身或半身动作）。',
              '- 画面重点：喷嘴长度比例、电池仓与握把转轴细节、轻便便携感；可用少量飞叶动势体现风力，但保持真实比例与商品结构一致。',
              '- 禁止点：改变主机结构与主色、误生成为吸尘器/割草机、出现多台不同型号混搭。',
              '',
