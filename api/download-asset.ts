function safeFilename(raw: string) {
  const name = String(raw || 'download')
    .replace(/[\\/:*?"<>|]+/g, '-')
    .replace(/\s+/g, ' ')
    .trim()
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    return res.status(500).json({ success: false, error: e?.message || '下载失败' })
  }
}
