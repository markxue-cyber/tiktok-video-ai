...
export default async function handler(req: any, res: any) {
  try {
    ...
    const resendType = String(type || 'signup')

    const tokenResp = await fetch(`${base}/auth/v1/resend`, {
      method: 'POST',
      headers: {
        apikey: anonKey,
    ...
    if (!tokenResp.ok) {
      const msg =
        data?.error_description || data?.error || data?.message || data?.msg || (typeof text === 'string' ? text : 'resend failed')
