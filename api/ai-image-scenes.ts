  ...
  try {
    ...
    if (!apiKey) {
      const scenes = KEYS.map((k) => ({
        ...
        description:
          k === 'commercial_white'
            ...
            : k === 'selling_focus'
              ...
              : k === 'lifestyle'
                ? '真实生活场景弱化背景，产品与使用情境结合；服装类真人穿搭须中景/半身且含完整头部，禁止无头裁切。'
                : k === 'comparison'
        ...
        imagePrompt:
          k === 'lifestyle'
            ? '在保持参考商品一致的前提下，强化场景生活图：生活化环境背景虚化；若真人穿着展示服饰，采用中景或半身，头顶完整在画面内，禁止裁到领口无头的构图。'
            : `在保持参考商品一致的前提下，强化「${TITLES[k]}」方向：光影与构图与其它场景要有明显差异，适合电商投放。`,
      }))
        ...
        messages: [
          {
            ...
            content: [
              ...
              '- lifestyle：可辨认生活/使用环境，背景虚化；释放 DNA 中未展开的生活联想，主体清晰。若类目为服饰/可穿戴且用真人展示：imagePrompt 须明确「中景或半身、完整头部入画（可背/侧视）」，严禁引导「无头」胸像裁切。',
              '- comparison：对比或效果暗示构图、信息层级；勿编造数据。',
              '- detail：角度/陈列/做工细节；背景干净，不大场景叙事。',
              '- atmosphere：允许强氛围光、冷暖对比、情绪环境；DNA 里压抑的「暗夜/霓虹/工业墙」气质优先在本条落地，但仍需商品可识别。含真人时同样须完整头颈肩，禁止无头裁切。',
              '',
              '叶吹机/手持鼓风机专项（若识别到该类目优先执行）：',
              '- commercial_white：输出标准结构主图，机身与喷嘴完整可见，背景白/浅灰干净。',
              '- selling_focus：优先写“180°旋转手柄、可拆电池、喷嘴接口/风道结构”等功能特写，构图可接近信息图但避免强制生成可读大段文字。',
              '- lifestyle：优先写“庭院/草坪/车道落叶清理”使用场景，可含真人操作，动作自然且比例真实。',
              '- comparison：写“便携/续航形态/使用灵活度”的对比构图（例如旧式笨重设备作为弱化参照），禁止编造具体参数与认证数据。',
              '',
              '硬约束：',
