...
export default async function handler(req, res) {
  ...
  try {
    ...
    const resp = await fetch(`${baseUrl()}/rest/v1/assets?id=eq.${encodeURIComponent(assetId)}&user_id=eq.${encodeURIComponent(userId)}`, {
      method: 'DELETE',
      headers: {
        apikey: serviceKey,
    ...
    if (!resp.ok) {
      const data = await parseJson(resp)
      return sendJson(res, 200, { success: false, error: data?.message || '删除失败', raw: data })
    }
