...
export async function requireActiveSubscription(req: any) {
...
export async function checkAndConsume(req: any, opts: { type: 'image' | 'video' | 'llm'; units?: number; relatedTaskId?: string; resultJson?: any }) {
  ...
  const limit =
    opts.type === 'image' ? limits.imagePerDay : opts.type === 'video' ? limits.videoPerDay : limits.llmPerDay
...
export async function finalizeConsumption(req: any, resultJson: any, relatedTaskId?: string) {
